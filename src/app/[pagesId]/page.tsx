          ...
          {pages.data.content &&
            pages.data.content.map((datas: Content, index: number) => {
              ...
              switch (getSectionName(datas.type)) {
                ...
                case "PageTemplate":
                  return <span key={`section_${index}`}></span>;
      ...
      {datas.data.rows.map((row: Row, index: number) => {
        const attributes: {
          ...
          className: `flex flex-wrap ${
            ...
            row.justifyContent
          } max-w-2xl  ${colSpacing} sm:mx-auto lg:-mx-8 lg:max-w-none xl:mx-0`,
        };
                  ...
                  {col.widget &&
                    col.widget.map((widget: Widget, index: number) => {
                      const WidgetComponent =
                        Widgets[
                          getWidgetName(widget.type) as keyof typeof Widgets
                        ];
                      return (
                        WidgetComponent && (
                          <WidgetComponent
